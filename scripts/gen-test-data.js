...
const DATES = getThisWeekDates(targetDate);
const NUM_BOOKINGS_PER_DAY = [3, 10]; // min/max

console.log(`Generating bookings for dates: ${JSON.stringify(DATES, null, 2)}\n`);
...
DATES.forEach(date => {
  const bookingsPerDay = faker.number.int({ min: NUM_BOOKINGS_PER_DAY[0], max: NUM_BOOKINGS_PER_DAY[1] });
  let attempts = 0;
  let created = 0;
  ...
  while (created < bookingsPerDay && attempts < 50) {
    ...
    const duration = faker.helpers.arrayElement([30, 45, 60, 90]); // Added 90 to match modal
...
writeFileSync("public/bookings-regular.json", JSON.stringify(regularBookings, null, 2));  // Regular Booking format

// Summary
console.log("\n✅ Generated:");
console.log(`   - ${therapists.length} therapists`);
console.log(`   - ${customers.length} customers`);
console.log(`   - ${bookings.length} total bookings across ${DATES.length} days`);
console.log("\n📅 Bookings per day:");
DATES.forEach(date => {
