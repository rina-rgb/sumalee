import type { Booking } from "../Calendar/Calendar";
...
export default function BookingModal({ isOpen, onClose, selectedBooking, formAction }: BookingModalProps) {
  ...
  const { firstName, lastName, email, phone, service, startTime, notes } = selectedBooking && 'firstName' in selectedBooking ? selectedBooking : {};

  return (
    <Dialog open={isOpen} onClose={onClose}>
      <form id="booking-form" action={formAction}>
        <Fieldset>
          <Legend>{isExistingBooking ? 'Edit Appointment' : 'Book Appointment'}</Legend>
          <Text>Please fill out the form below to {isExistingBooking ? 'update' : 'book'} your appointment for {startTime}:00.</Text>
          <FieldGroup>
